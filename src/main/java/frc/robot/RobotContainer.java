...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        joystick.rightTrigger().whileTrue(new ConditionalCommand(
                ...
                s_Shooter.slowShoot(), 
                () -> s_Elevator.isAtL4()
        )).onFalse(Commands.parallel(Commands.sequence(new WaitCommand(.25), driveWithJoystick()), s_Shooter.stop()));
       ...
       // joystick.leftStick().onTrue(s_Algae.intake()).onFalse(s_Algae.stopShooter());
        ...
        joystick.b().whileTrue(Commands.parallel(
                        new DriveToPose(drivetrain, new Transform2d(Units.inchesToMeters(-33.5/2-1.5), Units.inchesToMeters(0.5+2.25+.25), Rotation2d.fromDegrees(-3)), joystick, 8), 
                        s_Shooter.shoot()
                ))
