...
public class ReefBranchAlign extends Command {
	...
	public ReefBranchAlign(CommandSwerveDrivetrain drivetrain, Transform2d shift, DoubleSupplier joystickXInput) {
		...
		m_translationController = new ProfiledPIDController(4.0, 0.0, 0.1, new TrapezoidProfile.Constraints(7, 7));
	...
	public void initialize() {
		...
		ChassisSpeeds fieldVelocity = ChassisSpeeds.fromRobotRelativeSpeeds(m_drivetrain.getState().Speeds,
				initialPose.getRotation());

		m_translationController.reset(
				m_goalPose.relativeTo(initialPose).getTranslation().getY(),
				// Math.abs(initialPose.getTranslation().getY() -
				// m_goalPose.getTranslation().getY()),
	...
	public void execute() {
		...
		// m_thetaController.calculate(currentPose.getRotation().getRadians(),
		...
		double thetaVelocity = // m_thetaController.calculate(currentPose.getRotation().getRadians(),
								// m_goalPose.getRotation().getRadians());
		...
		final ChassisSpeeds CS = ChassisSpeeds.fromFieldRelativeSpeeds(
				new ChassisSpeeds(m_joystickInput.getAsDouble(), driveVelocity.getY(), thetaVelocity),
				currentPose.getRotation());// new ChassisSpeeds(m_joystickInput.getAsDouble(), driveVelocity,
											// thetaVelocity);

		m_drivetrain.setControl(m_drive.withSpeeds(
