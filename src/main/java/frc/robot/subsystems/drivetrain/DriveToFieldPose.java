...
public class DriveToFieldPose extends Command {
    ...
    public DriveToFieldPose(CommandSwerveDrivetrain drivetrain, Pose2d goalPose, CommandXboxController controller,double maxV) {
        m_goalPose = goalPose;
        m_controller = controller;
        m_translationController = new ProfiledPIDController(6, 0.0, 0.0, new TrapezoidProfile.Constraints(maxV, 4));
    ...
    public void execute() {
        ...
        Logger.recordOutput("Drivetrain/DriveToFieldPose/ChassisSpeeds", CS);

        Logger.recordOutput("Drivetrain/DriveToFieldPose/DistanceToGoalPose", distanceToGoalPose);
        Logger.recordOutput(
                "Drivetrain/DriveToFieldPose/Setpoint",
                new Pose2d(
                        m_lastSetpointTranslation, new Rotation2d(m_thetaController.getSetpoint().position)));
        Logger.recordOutput("Drivetrain/DriveToFieldPose/Goal", m_goalPose);
    }
