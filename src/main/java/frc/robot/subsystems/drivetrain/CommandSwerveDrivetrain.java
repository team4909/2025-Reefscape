		...
		if (DriverStation.getAlliance().isPresent()) {
			if (DriverStation.getAlliance().get() == DriverStation.Alliance.Red) {
				for (int i = 0; i < centerFaces.length; i++) {
					centerFaces[i].getTranslation().rotateAround(
							new Translation2d(VisionConstants.aprilTagLayout.getFieldLength() / 2,
									VisionConstants.aprilTagLayout.getFieldWidth() / 2),
							Rotation2d.k180deg);
